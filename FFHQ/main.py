...
import utils
from ffhq_data import get_att_name_list
from sampling import _sample_q_dict, CCF
from eval_sampler import test_clf_acc, ConditionalSampling, SequentialEditing

LOAD_PATHS = lambda att_name: f'./pretrained/dense_embedder_w/best_valid_ckpt_{att_name}.pt'


def main(args):
    tag = f'{args.att_names}_{args.seq_len}/{args.sample_method}_{args.att_source}_bs{args.batch_size}' \
          f'_trunc{args.truncation}_regz{args.reg_z}_zinit{int(args.use_z_init)}_regid{args.reg_id}' \
    ...
    # conditional latent traversal (use-specified)
    if args.cond_latent_traversal:
        print('starting cond_latent_traversal...')
        z_inits = utils.get_z_inits(args.traversal_num, args.batch_size, latent_dim=args.latent_dim, device=device)
        for i, z_init in enumerate(z_inits):
            cond_sampling.get_samples(att_vals=args.att_vals, discrete_num=args.discrete_num, trim_num=args.trim_num,
                                      z_init=z_init, subdir=f'zh{i:03d}')

    # eval cond acc
    if args.eval_cond_acc:
        print('starting eval_cond_acc...')
        cond_sampling.eval_acc(att_source=args.att_source)

    # eval cond fid
    if args.eval_cond_fid:
        print('starting eval_cond_fid...')
        cond_sampling.eval_fid(att_source=args.att_source, inception_pkl=args.inception_pkl)
    ...
    # eval seq_edit acc
    if args.eval_seq_edit_acc:
        print('starting eval_seq_edit_acc...')
        seq_editing.eval_acc(att_source=args.att_source)

    # eval seq_edit fid
    if args.eval_seq_edit_fid:
        print('starting eval_seq_edit_fid...')
        seq_editing.eval_fid(att_source=args.att_source, inception_pkl=args.inception_pkl)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="ffhq_latent_train", type=str,
                        choices=["ffhq_latent_train", "ffhq_latent_test"],
    ...
    parser.add_argument("--inception_pkl", type=str, default='./pretrained/metrics/inception_ffhq_gen_1k.pkl')
    ...
    parser.add_argument("--att_source", type=str, default='training')
    # ODE
    ...
    parser.add_argument("--traversal_num", type=int, default=5)
    ...
    parser.add_argument("--eval_seq_edit_acc", action="store_true")
    parser.add_argument("--eval_cond_fid", action="store_true")
    parser.add_argument("--eval_seq_edit_fid", action="store_true")
    parser.add_argument("--cond_use_case", action="store_true")
    parser.add_argument("--seq_edit_use_case", action="store_true")
    parser.add_argument("--cond_latent_traversal", action="store_true")
