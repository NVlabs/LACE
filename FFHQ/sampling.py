...
from ffhq_data import get_att_name_list, get_n_classes_list
from stylegan2 import Generator
from latent_model import DenseEmbedder


NETWORK_PKL = './pretrained/stylegan2_pt/stylegan2-ffhq-config-f.pt'


class F(nn.Module):
    def __init__(self, att_names='gender', im_sz=1024, latent_dim=512, truncation=1, subset_selection=False):
        ...
        g_ema = Generator(size=im_sz, style_dim=latent_dim, n_mlp=8)
        checkpoint = torch.load(NETWORK_PKL)
        g_ema.load_state_dict(checkpoint["g_ema"])
        self.G = g_ema
        mean_w = g_ema.mean_latent(n_latent=2048)
        ...
        def mapping_z_to_w(z):
            assert z.shape[-1] == g_ema.style_dim, z.shape
            w = g_ema.style(z)  # no truncation

            if truncation < 1:
                truncation_latent = mean_w.to(z.device)
    ...
    def generate_images(self, g_z_sampled, seq_indices=[], update_ws_prev=False, is_detached=True):
        ...
        img, _ = self.G([ws], randomize_noise=False, input_is_latent=True)
