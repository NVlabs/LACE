...
from afhq_data import get_att_name_list, get_n_classes_list
from latent_model import DenseEmbedder


NETWORK_PKL = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada/pretrained/afhqcat.pkl'
...
class F(nn.Module):
    ...
    def generate_images(self, g_z_sampled, seq_indices=[], update_ws_prev=False, is_detached=True):
        ...
        ws = g_z_sampled
        if ws.ndim == 2:
            ws = ws.unsqueeze(1).repeat([1, 16, 1])  # bs x n_mappings x 512

        if self.subset_selection:
            ws = self.apply_subset_selection(ws, seq_indices, update_ws_prev=update_ws_prev)

        assert ws.shape[1:] == (self.G.num_ws, self.G.w_dim), (self.G.num_ws, self.G.w_dim)
