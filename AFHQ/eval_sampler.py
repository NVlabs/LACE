...
class ConditionalSampling(Sampling):
    def __init__(self, att_names, sampler, batch_size, latent_dim, ccf, device, root_path, save_path, ode_kwargs,
                 ld_kwargs, sde_kwargs, every_n_plot=5, every_n_print=5, use_z_init=False, dis_temp=1., mode='cond'):
        ...
        print(f'mode: {mode}, dis_temp: {dis_temp}')
...
def test_clf_acc(ccf, att_names, dataset, root_path, batch_size, device):
    ...
    if dataset == "afhqcat_latent_train":
        dset = AFHQCatLatentDataset(root_path, att_names, train=True)
        clf_func = ccf.classify_x
    elif dataset == "afhqcat_latent_test":
        dset = AFHQCatLatentDataset(root_path, att_names, train=False)
        clf_func = ccf.classify_x
    else:
        raise NotImplementedError
